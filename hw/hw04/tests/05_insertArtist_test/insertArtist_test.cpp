...
TEST(hw03Test, insertArtistTest) {
  ARTIST *a = nullptr;
  AENTRY *e = nullptr;
  ALIST *l = nullptr;
  ...
  EXPECT_TRUE(equalVectorsbyArtistName(ll2vec, artist_name)) << "\nArtist names are not inserted in the correct order.";

  if(HasFailure()) {
    cerr << "Artist linked list construction with insert before/after has failed" << endl;
  } else {
