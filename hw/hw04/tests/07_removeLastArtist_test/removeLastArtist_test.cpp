...
TEST(hw03Test, removeLastTest) {
  ...
  vector<string> artist_name{name1, name2, name3};
  vector<ARTIST> ll2vec = linkedList2Vector(l);

  EXPECT_EQ(l->length, 3) << "\nExpect list length of 3 when 4 artist records are added to the artist list and the last is removed.";
  EXPECT_TRUE(equalVectorsbyArtistName(ll2vec, artist_name)) << "\nFinal list artist names are not in the correct order.";

  if(HasFailure()) {
    cerr << "Remove last artist from linked list has failed" << endl;
  } else {
