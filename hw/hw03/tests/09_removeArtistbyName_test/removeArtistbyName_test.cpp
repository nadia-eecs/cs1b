...
TEST(hw03Test, removeArtistNameTest) {
  ARTIST *a = nullptr;
  ALIST *l = nullptr;
  ...
  removeArtistbyName(l, name2);
  EXPECT_EQ(l->length, 3) << "\nOriginal list having a length of 4 should now be of length 3";

  if(HasFailure()) {
    cerr << "Remove artist by name in the linked list has failed" << endl;
  } else {
