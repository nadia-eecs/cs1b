...
TEST(hw03Test, prependArtistTest) {
  ARTIST *a = nullptr;
  ALIST *l = nullptr;
  ...
  vector<string> artist_name{name4, name3, name2, name1};
  ...
  EXPECT_TRUE(equalVectorsbyArtistName(ll2vec, artist_name)) << "\nArtist names are not prepended in the correct order.";

  if(HasFailure()) {
    cerr << "Artist linked list construction with prepend has failed" << endl;
  } else {
