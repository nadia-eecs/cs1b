...
TEST(hw03Test, removeFirstArtistTest) {
  ARTIST *a = nullptr;
  ALIST *l = nullptr;
  ...
  vector<string> artist_name{name2, name3, name4};
  vector<ARTIST> ll2vec = linkedList2Vector(l);

  EXPECT_EQ(l->length, 3) << "\nExpect list length of 3 when 4 artist records are added to the artist list and the first is removed.";
  EXPECT_TRUE(equalVectorsbyArtistName(ll2vec, artist_name)) << "\nremove first artist in linked list is not in the correct order.";

  if(HasFailure()) {
    cerr << "Remove first artist from linked list has failed" << endl;
  } else {
